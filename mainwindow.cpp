...
void MainWindow::initWidget()
{
    ...
    /* 图片窗口*/
    backGround = new QLabel(videoWidget);
    backGround->setPixmap(QPixmap("./icons/music.png"));
    //backGround->setStyleSheet("border-image:none;" "background: transparent;""border:none");
    //backGround->setScaledContents(true);
    backGround->setMinimumSize(500, 480);
    //backGround->show();
    ...
    //hBoxLayout[0]->addWidget(backGround);   //图片窗口
...
void MainWindow::SltBtnAddFile()
{
    QStringList filter; // 定义过滤器
        /* 包含所有 xx 后缀的文件 */
    filter << "*.mp4" << "*.mkv" << "*.wmv" << "*.avi" << "*.mp3";
    QStringList filePaths = QFileDialog::getOpenFileNames(this, "选择多个文件","./", "*.mp4 *.mkv *.wmv *.avi *.mp3");
...
void MainWindow::SltBtnRateClicked()
{
    ...
    {
        videoPlayer->pause();
        pushButton[7]->setText(QString::fromLocal8Bit("倍速×5"));
        videoPlayer->setPlaybackRate(5);
        rate = 5;
        videoPlayer->play();
    }
    else if (rate == 5)
